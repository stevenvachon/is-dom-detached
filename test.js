...
it("returns false for a non-Node", runInBrowser(() =>
{
	...
	fixtures.forEach(fixture => expect(isDetachedNode(fixture)).to.be.false);
}));
...
it("supports same- and cross-Realm Document Nodes", runInBrowser(() =>
{
	...
	[
		...
		{
			node: anotherDocument.createDocumentFragment(),
			result: true,
		},
		{
...
it("supports seldom-used Nodes", runInBrowser(() =>
{
	const nodes =
	[
